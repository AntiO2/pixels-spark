...
STATE_DIR="${STATE_DIR:-$(pixels_get_property pixels.cdc.chbenchmark.${PROFILE}.state-dir /home/ubuntu/disk1/tmp/chbenchmark_w1_cdc_state)}"
LOG_DIR="${LOG_DIR:-$(pixels_get_property pixels.cdc.chbenchmark.${PROFILE}.log-dir /home/ubuntu/disk1/tmp/chbenchmark_w1_cdc_logs)}"
CKPT_ROOT="${CKPT_ROOT:-$(pixels_get_property pixels.cdc.chbenchmark.${PROFILE}.checkpoint-root /home/ubuntu/disk1/tmp/chbenchmark_w1_cdc_ckpt)}"
METRICS_DIR="${METRICS_DIR:-$(pixels_get_property pixels.cdc.chbenchmark.${PROFILE}.metrics-dir /home/ubuntu/disk1/tmp/chbenchmark_w1_cdc_metrics)}"
RESOURCE_DIR="${RESOURCE_DIR:-$(pixels_get_property pixels.cdc.chbenchmark.${PROFILE}.resource-dir ${ROOT_DIR}/data/chbenchmark/w1/resource)}"
SPARK_EVENTS_DIR="${SPARK_EVENTS_DIR:-$(pixels_get_property pixels.spark.event-log.dir /home/ubuntu/disk1/tmp/spark-events)}"
TARGET_ROOT="${TARGET_ROOT:-$(pixels_get_property pixels.cdc.chbenchmark.${PROFILE}.target-root "$(pixels_get_property pixels.import.chbenchmark.${PROFILE}.target-root s3a://home-zinuo/deltalake/chBench_wh1)")}"
DATABASE="${DATABASE:-$(pixels_get_property pixels.cdc.chbenchmark.${PROFILE}.database "$(pixels_get_property pixels.cdc.database pixels_bench)")}"
CDC_BENCHMARK="${CDC_BENCHMARK:-$(pixels_get_property pixels.cdc.chbenchmark.${PROFILE}.benchmark chbenchmark)}"
...
main() {
  ...
  log "all table jobs submitted profile=chbenchmark_${PROFILE} unit_prefix=${UNIT_PREFIX}"
}
